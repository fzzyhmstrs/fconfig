...
Migration will follow many of same the basic steps as [New Configs](New-Configs), so check that section out as needed.
...
### Validation
Fzzy Config has many validation options. Validation is a foundation of its very functionality! Whether you utilize these tools purposefully or not, Fzzy Config will apply them in the background to keep everything in your config running smoothly.

[➡️ Validation](../config-concepts/Validation)

### Annotations
Fzzy Config has a powerful suite of Annotations to add the depth of function you need to your refactored config.

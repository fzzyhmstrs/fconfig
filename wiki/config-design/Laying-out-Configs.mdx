...
* _This choice will also affect what the player sees when they open your 'root' config screen._
...
* _Don't worry about adding new setting 'in the middle' of a config class later. Fzzy Config will handle the new addition without a hiccup. Focus on keeping your settings organized as your config grows_
...
* Take advantage of class-wide annotations for a particular config like [`@WithPerms`](https://github.com/fzzyhmstrs/fconfig/wiki/Annotations#withperms-)/[`@WithCustomPerms`](https://github.com/fzzyhmstrs/fconfig/wiki/Annotations#withcustomperms-) (in this case, lock some settings in a config that requires particular permissions)
...
## 2. Organization
Large configs benefit from being broken up into parts, if separate config classes aren't suitable. Fzzy Config supports organization in three primary ways for two main purposes, sub-configuration and layout flow. Two of these methods also act as _anchors_ ([See below](https://github.com/fzzyhmstrs/fconfig/wiki/Laying-out-Configs#anchors)).
| Strategy | UI Style | Repeatable | Anchor |
|----------|----------|------------|--------|
| [Sections](https://github.com/fzzyhmstrs/fconfig/wiki/Laying-out-Configs#sections) | Opens new screen | Yes | Yes|
| [Objects](https://github.com/fzzyhmstrs/fconfig/wiki/Laying-out-Configs#objects) | Opens popup | Yes | No |
| [Groups](https://github.com/fzzyhmstrs/fconfig/wiki/Laying-out-Configs#groups) | Collapsible inline grouping | No | Yes |

If you use a common subclass for creation of repeating sub-elements, consider using the [`@Translation`](https://github.com/fzzyhmstrs/fconfig/wiki/Annotations#translation-) annotation, which will let you define common lang for all instances of the repeating element. 

### Sections
Subconfig classes can be extended from [`ConfigSection`](https://fzzyhmstrs.github.io/fconfig/-fzzy%20-config/me.fzzyhmstrs.fzzy_config.config/-config-section). Subconfigs made this way will open a new, separate config screen of their own on top of the parent config screen.
...
## 3. Anchors
Anchors are a concept introduced in Fzzy Config 0.6.0. Much like anchors in webpages, anchors are "points of interest" that you can fast-navigate to from within the config GUI. Back in [Organization](https://github.com/fzzyhmstrs/fconfig/wiki/Laying-out-Configs#organization), two of three anchors were introduced. The third is of course configs themselves.
* Configs
* Config Sections
* Groups

### _Result Providers are an experimental API as of 0.5.3_
_Stable release target is 0.6.0_
...
### Result Provider
A result provider is an interface with one job: provide a result based on a passed config `scope`. The [Translation page](https://github.com/fzzyhmstrs/fconfig/wiki/Translation#example) has a decent overview of what `scope` is in fzzy configs context. It works similarly to a web address, or forge/luckperms permission ids.
```kotlin !!tabs Kotlin
...
Currently, the drop is ward-wired to exactly the same random chance as a wither skull.
...
#### Float Provider
To make this table dynamic, we will need a provider of float values to feed into our loot number provider. _This provider could be improved to accept any number type, but that is beyond the scope of this example_
```kotlin !!tabs Kotlin
...
_Methods that are unnecessary to illustrating this example are excluded. Assume that things like the codec, type, and so on are implemented._
```kotlin !!tabs Kotlin
...
private static final ResultProvider<Float> floatProvider = ConfigApi.result().createSimpleResultProvider(0f, Float.class);
}
```

#### Json Integration
Let's assume our presumptive type and codec are registered under the identifier `my_mod:chance_provider` and takes one string `scope`. The number provider json would thus look like:
```json
{
...
#### The Config
Our config, registered under the identifier `my_mod:my_config` needs the corresponding settings we want to refer to.

It might also be a good idea to include the looting multiplier. _(And an even better idea to move these into a separate object for organization, but that's outside the example)_
```kotlin !!tabs Kotlin
...
#### Table Integration
All the pieces can now be tied together. Our new provider condition can be used in loot tables.

This table will now have its chance dynamically updated from the config definition. Updates to the config values will be applied right away, not even needing a resource reload or similar.
```json
{
  "type": "minecraft:entity",
  "pools": [

...
### Creation
To define an action button, simply add a `ConfigAction` as a property/field in the config class. The config GUI will gain an action button in the order that you place the action in the classes fields, just like any other setting. It might be helpful for users if you place your special buttons together, and at the top or bottom. Or maybe, place them in an [object](https://github.com/fzzyhmstrs/fconfig/wiki/Laying-out-Configs#objects) and have all your special actions in one convenient popup!

[`IgnoreVisibility`](https://github.com/fzzyhmstrs/fconfig/wiki/Annotations#ignorevisibility-) can be used with action just like any other config field. Also like any other config field, it **can't be final**.

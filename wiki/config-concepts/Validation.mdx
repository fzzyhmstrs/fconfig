...
### Conditions
`ValidatedBoolean` can be used directly as an input into a `ValidatedCondition`. See [Conditional Settings](#conditional-settings-) to learn more.
...
## Conditional Settings [⤴](#validation-options)
...
## Codecs [⤴](#validation-options)
Any `ValidatedField` instance can be used to generate a `Codec` of the type the validation represents. Importantly, that resulting Codec will be backed by all of the validation present in the field.

...
* _If you try to add a setting of an unsupported type, you will need to create custom validation for it, or it **will not** appear in config GUIs nor will be serialized in the config files_

For more specific control of your settings, in the vein of Minecraft `GameOption`, specifically define the validation for your setting using one of the options above, or a custom implementation if you are feeling adventurous. This grants you the ability to:
...
Any validation can be wrapped with conditions that define whether the setting is "active" or not. This allows you to create settings that are based on game state, the state of other settings, and so on:
...
You can listen to changes of any `ValidatedField` (actually of any `Entry`, of which validated fields are the most common) by supplying a consumer of that field.

Listening works on both server and client and will trigger on config load too, as applicable. The listeners apply any time the stored value is set by anything (with a couple exceptions, see below)

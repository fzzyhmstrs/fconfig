...
You don't need to supply translations for all of these (or any of these), missing entries will either be ignored or a fallback used
</Callout>
...
This annotation is useful for repeating parts. If you have a config object that repeats many times in a config; normally you would have to provide a translation for every individual setting. That would get annoying fast. `@Translation` allows you to define one lang prefix that is common for all instances of the annotated field.

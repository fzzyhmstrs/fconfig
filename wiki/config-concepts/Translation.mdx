...
### `translationProvider`, `descriptionProvider`
Validated settings can be provided with a custom name and/or tooltip via a `translationProvider`/`descriptionProvider`. This may or may not be an actual translated text, it all depends on the implementation of the provider. In general, it is recommended to use translatable text wherever possible, even in a custom provider.

Learn about these [here.](Validation#translationprovider)
...
## <div><ArrowDownWideNarrowIcon height={26} width={22} style={{display: "inline", color: "#B2BFF9", paddingTop: "4px", verticalAlign: "top"}} /> Hierarchy</div>
All the name, description, and prefix options fit into a 4-tier hierarchy, top-to-bottom. The first encountered valid text is the one used.
1. `@Translation` - texts defined with this annotation override any other type of text generation
2. `translationProvider`, `descriptionProvider` - a custom name or tooltip will take precedence over built in translations
3. `my_mod.my_config.my_section.myField` Translation - the baseline translation system
4. Fallback - if no valid translation mechanisms are otherwise provided, a fallback as described above will be used

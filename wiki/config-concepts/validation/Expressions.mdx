...
Fzzy Config has a built-in math engine called `Expression`. See the [Expressions](/features/Expressions) article for more details. Validating expressions works much like any other validation, `ValdiatedExpression` wraps a default expression and allowable characters into a `ValidatedField` with GUI support, error correction, and so on.

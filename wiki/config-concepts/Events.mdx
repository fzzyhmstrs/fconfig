As of Fzzy Config 0.5.0 there is an event system, with a series of directly implementable methods in `Config` for managing events directly in your own config class, an `EventApi` for registering to various generic config events, as well as a way to attach listeners directly to [Validation](https://github.com/fzzyhmstrs/fconfig/wiki/Validation)
...
<Callout>
**NOTE:** These changes may be made on any thread, depending on the particulars of how the field is interacted with. Take precautions when interacting with game state.
</Callout>

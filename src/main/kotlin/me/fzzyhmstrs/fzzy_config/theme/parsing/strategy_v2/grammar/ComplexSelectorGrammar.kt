...
object ComplexSelectorGrammar: TokenConsumer<List<Token<*>>> {

    override fun consume(queue: TokenQueue, args: Set<String>): ValidationResult<List<Token<*>>> {
        if (!queue.canPoll()) return ValidationResult.error(listOf(), "Can't consume a complex selector from an empty queue")

 ...
 * @since 0.1.0
 */
class ParseResult<T> private constructor(private val storedVal: T, private val error: String = "") {
    /**
    ...
    companion object {
         ...
         * @since 0.1.0
         */
        fun <T> success(storedVal: T): ParseResult<T> {
            return ParseResult(storedVal)
        }
         ...
         * @since 0.1.0
         */
        fun <T> error(storedVal: T, error: String): ParseResult<T> {
            return ParseResult(storedVal, error)
        }
         ...
         * @since 0.2.0
         */
        fun <T>predicated(storedVal: T, valid: Boolean, error: String): ParseResult<T> {
            return if(valid) ParseResult(storedVal) else ParseResult(storedVal, error)
        }
         ...
         * @since 0.2.0
         */
        fun <T> ParseResult<*>.wrap(newVal: T): ParseResult<T> {
            return ParseResult(newVal, this.error)
        }
         ...
         * @since 0.2.0
         */
        fun <T> ParseResult<T>.also(newTest: Boolean, error: String): ParseResult<T> {
            return if(!newTest) {
                ...
                ParseResult(this.storedVal, totalError)
            } else {
                this
            }
        }
         ...
         * @since 0.2.0
         */
        fun <T> ParseResult<T>.report(errorBuilder: MutableList<String>): ParseResult<T> {
         ...
         *
         */
        fun <N, T> ParseResult<T>.map(to: Function<T, out N>): ParseResult<N> {
            return ParseResult(to.apply(this.storedVal), this.error)
        }

        fun <T: Any> ParseResult<ConfigContext<T>>.contextualize(): ParseResult<T> {

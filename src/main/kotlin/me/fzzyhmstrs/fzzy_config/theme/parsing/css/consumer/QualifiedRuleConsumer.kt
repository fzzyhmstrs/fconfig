...
object QualifiedRuleConsumer: TokenConsumer<Token<*>> {
    ...
    override fun consume(queue: TokenQueue, args: Set<String>): ValidationResult<Token<*>> {
        ...
        fun complete(value: StyleBlockConsumer.StyleBlock): ValidationResult<Token<*>> {
            return ValidationResult.ofMutable(Token(
                CssType.QUALIFIED_RULE,
                QualifiedRule(TokenQueue.ofFiltered(prelude), value),
                initial.line(),
                initial.column()), errors)
        }
        while (queue.canPoll()) {
            val peek = queue.peek()
            when (peek.type) {
                Parser.EOF -> {
                    queue.poll()
                    return ValidationResult.error(ListOfRulesConsumer.unknownRule(initial.line(), initial.column()), Errors.END_OF_FILE, "Qualified rul")
                }
                CssType.OPEN_BRACE -> {

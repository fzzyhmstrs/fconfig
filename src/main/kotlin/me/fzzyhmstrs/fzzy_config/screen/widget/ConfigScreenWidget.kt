...
{
    init {
        setPosition(position.positionX(Minecraft.getInstance().window.guiScaledWidth), position.positionY(Minecraft.getInstance().window.guiScaledHeight))
    }

    private fun getTex(): Identifier {
        if(!active)
            return TextureIds.CONFIG_INACTIVE
        return if (isHovered || isFocused)
    ...
    override fun renderCustom(context: GuiGraphicsExtractor, x: Int, y: Int, width: Int, height: Int, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        setTooltip(if (this.active) {
            Tooltip.create(TextureIds.CONFIG_LANG)
        } else {
            Tooltip.create(TextureIds.CONFIG_INACTIVE_LANG)
        })
    }

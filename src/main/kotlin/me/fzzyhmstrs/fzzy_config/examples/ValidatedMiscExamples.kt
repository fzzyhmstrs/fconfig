...
object ValidatedMiscExamples {
    ...
    fun registries() {
        ...
        //Type: ValidatedField<Item>
        var validatedItemComplex = ValidatedRegistryType.of(Items.WOODEN_SWORD, BuiltInRegistries.ITEM) { id, re -> id.namespace == "minecraft" && re.value() is ProjectileWeaponItem }

        //Since we know all the outputs will be swords, we could map it down to provide RangedWeaponItem outputs
        //Type: ValidatedField<RangedWeaponItem>
        var validatedItemSword = ValidatedRegistryType.of(Items.WOODEN_SWORD, BuiltInRegistries.ITEM) { id, re ->
            id.namespace == "minecraft" && re.value() is ProjectileWeaponItem
        }.map(
            { item -> item as ProjectileWeaponItem },
            Function.identity()
        )
    }
}